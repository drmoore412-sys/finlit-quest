...
function updateDashboard(){
  ...
  const playerLevel=Math.floor(player.xp/100)+1,levelXp=player.xp%100,activeIndex=Math.min(state.review?Math.max(0,state.level-1):state.level,JOURNEY_SECTIONS.length-1),journey=JOURNEY_SECTIONS[activeIndex];
  $("#dashboardXp").textContent=player.xp;$("#journeyCoins").textContent=0;$("#journeyGems").textContent=0;$("#journeyLevel").textContent=playerLevel;$("#journeyLevelProgress").style.width=`${levelXp}%`;
  ...
  $("#badgeCount").textContent=`${player.achievements.length} / 2`;$("#xpToNext").textContent=`${levelXp} / 100`;$("#footerProgressFill").style.width=`${levelXp}%`;
